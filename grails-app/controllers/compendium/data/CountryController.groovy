...
class CountryController {
    ...
    def show = {
        ...
        if (!countryInstance) {
            ...
            def idList = countryInstance.projects.id
            def plist = Project.listOrderBySession(order: "desc")
            def projects = []
            for (p in plist) {
                if (idList.contains(p.id)) projects.add(p)
            }
            [countryInstance: countryInstance, projects: projects]
        }
    }
